...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	WriteBuffer[0] = reg | 0b0000010;
	WriteBuffer[1] = (u8)val;
	XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, 2);
	if(ReadBuffer[1])xil_printf("maxreg_wr done goofed\r\n");
	XSpi_SetSlaveSelect(&SpiInstance, 0x0000);
}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
	for(int i = 0; i < nbytes; i ++)MAXreg_wr(reg,data[i]);
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	XSpi_SetSlaveSelect(&SpiInstance, 0x0001);
	WriteBuffer[0] = reg;
	if(XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, 2) != XST_SUCCESS)xil_printf("maxreg_wr done goofed\r\n");
	XSpi_SetSlaveSelect(&SpiInstance, 0x0000);
	return ReadBuffer[1];
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
	for(int i = 0; i < nbytes; i ++)data[i] = MAXreg_rd(reg);
	return (data + nbytes);
}
